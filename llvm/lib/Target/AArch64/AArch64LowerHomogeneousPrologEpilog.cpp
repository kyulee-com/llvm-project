...
static std::string getFrameHelperName(SmallVectorImpl<unsigned> &Regs,
  ...
  case FrameHelperType::PrologFrame:
    RegStream << "OUTLINED_FUNCTION_PROLOG_FRAME_";
    break;
...
///    ret
///
/// 2) _OUTLINED_FUNCTION_PROLOG_FRAME_x19x20x21x22:
///    stp x20, x19, [sp, #-16]!
///    stp x22, x21, [sp, #-16]!
///    add fp, sp, #32
///    ret
...
static Function *getOrCreateFrameHelper(Module *M, MachineModuleInfo *MMI,
                                        ...
                                        FrameHelperType Type) {
  assert(Regs.size() >= 2 && Regs[0] == AArch64::LR && Regs[1] == AArch64::FP);
  if (Type == FrameHelperType::Prolog || Type == FrameHelperType::PrologFrame) {
    // Prolog helpers do not store FP/LR
    Regs.erase(Regs.begin());
    Regs.erase(Regs.begin());
  }

  auto Name = getFrameHelperName(Regs, Type);
...
/// Get a valid positive adjustment to set fp from sp.
/// @param MBBI instruciton setting fp from sp.
/// @return a valid positive adjustment. Or 0 for any other case.
...
static bool shouldUseFrameHelper(MachineBasicBlock &MBB,
  ...
  case FrameHelperType::PrologFrame: {
    ...
    if ((FpAdjustment == 0) || (FpAdjustment != (RegCount - 2) * 8))
      return false;
...
bool AArch64LowerHomogeneousPE::lowerHOM_Epilog(
  ...
  if (Size == 0)
    return false;
  // Allow compact unwind case only for now.
  assert(Size % 2 == 0);
...
///    =>
///    stp x29, x30, [sp, #-16]!
///    bl _OUTLINED_FUNCTION_PROLOG_FRAME_x19x20x21x22
///    ...                                          ; NextMBBI
///
/// 2. With a helper
...
bool AArch64LowerHomogeneousPE::lowerHOM_Prolog(
  ...
  if (shouldUseFrameHelper(MBB, NextMBBI, Regs, FrameHelperType::PrologFrame)) {
    // The first store for FP/LR cannot be outlined.
    emitHomogeneousStore(MF, MBB, MBBI, *TII, Regs[0], Regs[1]);
    auto PrologFrameHelper =
        getOrCreateFrameHelper(M, MMI, Regs, FrameHelperType::PrologFrame);
    BuildMI(MBB, MBBI, DL, TII->get(AArch64::BL))
