...
static Expected<std::string> expandBundle(const StringRef InputPath) {
  ...
  if (BundlePaths.size() == 1)
    return BundlePaths.front();
  // Can only take exactle one object.
  return make_error<InstrProfError>(
      instrprof_error::unable_to_correlate_profile);
}
...
InstrProfCorrelator::get(const StringRef DebugInfoFilename) {
  auto FilenameOrErr = expandBundle(DebugInfoFilename);
  if (auto Err = FilenameOrErr.takeError())
    return std::move(Err);
  auto BufferOrErr = errorOrToExpected(MemoryBuffer::getFile(*FilenameOrErr));
