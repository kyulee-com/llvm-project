...
Error LTO::runThinLTO(AddStreamFn AddStream, FileCache Cache,
  ...
  // auto Run = [&](std::unique_ptr<ThinBackendProc> &BackendProcess) -> Error {
  auto Run = [&](ThinBackendProc *BackendProcess) -> Error {
  ...
  if (Error E = Run(FirstRoundLTO.get()))
    return E;
  ...
  Error E = Run(SecondRoundLTO.get());
