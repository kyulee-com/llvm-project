...
class StructuralHashImpl {
  ...
  std::unique_ptr<IndexPairOpndHashMap> IndexPairOpndHash = nullptr;
...
public:
  ...
  explicit StructuralHashImpl(bool DetailedHash,
    ...
    if (IgnoreOp) {
      ...
      IndexPairOpndHash = std::make_unique<IndexPairOpndHashMap>();
    }
  }
  ...
  stable_hash hashInstruction(const Instruction &Inst) {
    ...
    for (unsigned OpndIdx = 0; OpndIdx < Inst.getNumOperands(); ++OpndIdx) {
      ...
      if (IgnoreOp && IgnoreOp(&Inst, OpndIdx)) {
        ...
        IndexPairOpndHash->insert({{InstIdx, OpndIdx}, OpndHash});
      } else
  ...
  std::unique_ptr<IndexPairOpndHashMap> getIndexPairOpndHashMap() {
    return std::move(IndexPairOpndHash);
  }
};
